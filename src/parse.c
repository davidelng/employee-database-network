...
int validate_db_header(int fd, dbheader **headerOut) {
	...
	// network to host (ntoh) is needed because not all machine have the same
	// endiness 0x00000001 could be 0x010000000 on another machine
...
int delete_employee(dbheader *dbhdr, employee **employeesOut, unsigned int deleteID) {
	...
	for (; i < dbhdr->count + 1; i++) {
		...
		strncpy(newEmployees[j].name, (*employeesOut)[i].name,
						sizeof(newEmployees[j].name));
		strncpy(newEmployees[j].address, (*employeesOut)[i].address,
						sizeof(newEmployees[j].address));
		j++;
	}
